...
import { MOCK_SPLITS, WorkoutSplit } from "@/constants/split";
import { useUser } from "@/context/userContext";
import { loadSplit, saveSplit } from "@/utils/splitStorage";
...
export default function SplitDropdown() {
  ...
  const handleImportSplit = async () => {
    ...
    try {
      ...
      const split = await apiFetch<WorkoutSplit>(`/split/shared/${token}`, {
        method: "GET",
      });
  ...
  const createSplitFromPreset = async (split: WorkoutSplit) => {
    const res = await apiFetch<WorkoutSplit>("/split/create", {
      method: "POST",
      body: {
  ...
  const splitsToShow = search.trim().length > 0 ? searchResults : MOCK_SPLITS;
  ...
  return (
    <View className="w-[100%] mx-auto mt-4">
      ...
      <View className="absolute z-40 right-6 left-6 mb-40">
        ...
        {open && (
          <ScrollView className="bg-zinc-900 mt-2 rounded-lg max-h-80 border border-zinc-700">
            ...
            {/* ---------- SEARCH LOADER ---------- */}
            {searching && (
              <Text className="text-zinc-400 text-center py-4">Searching…</Text>
            )}
      ...
      >
        {/* ---------- WEEK CALENDAR ---------- */}
        <WeekCalendar onDaySelect={(d) => setSelectedDay(d)} />
          ...
          <View className="w-[90%] mx-auto mt-3 flex-row gap-3">
            ...
            >
              {sharingSplit ? (
                ...
                <Text className="text-black font-ScienceGothic text-center">
                  Share
                </Text>
              )}
            </TouchableOpacity>
